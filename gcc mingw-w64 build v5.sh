...
cd
rm -rf mingw-gcc-13.1.0/ mingw-builds/
...
#################################################
# start gcc build
# edit gcc, build-crt and mingw-w64 rt-versions. turn off computer sleep settings and possibly anti-virus.
# switchiing from msvcrt to ucrt builds can cause strange link-time failures (undefined references) which can be resolved with a clean restart of the build
cd && cd mingw-builds
./build --help
...
# temporarily change the toolchain download URL since a recent host for posix-sjlj-msvcrt is not available
...
./build --mode=gcc-$GCC_VER --arch=i686 --threads=posix --exceptions=sjlj --enable-languages=c,c++,fortran --with-default-msvcrt=$BUILD_CRT --rt-version=$MINGW64_CRT --jobs=4 --rev=$REV
...
export BUILD_CRT='ucrt'
./build --mode=gcc-$GCC_VER --arch=x86_64 --threads=posix --exceptions=seh --enable-languages=c,c++,fortran --with-default-msvcrt=$BUILD_CRT --rt-version=$MINGW64_CRT --jobs=4 --rev=$REV
#################################################
