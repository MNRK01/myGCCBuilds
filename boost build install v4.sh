...
# obtain variables from the calling environment
# GCC_VER="11.1.0"
...
# BOOST_VER="1_76_0"
if [[ ! $BOOST_VER ]]; then echo "Boost version not declared. Exiting..."; exit 1; fi
BOOST_VER_DOT=${BOOST_VER//_/.}
BOOST_VER_SHORT=$( echo $BOOST_VER | sed -e "s/_[0-9]$//" )
# ICU_VER="69.1"
if [[ ! $ICU_VER ]]; then echo "ICU version not declared. Exiting..."; exit 1; fi
...
if [[ ! -d /c/OSRC/boost/boost_$BOOST_VER ]]; then
    ...
    [[ -e boost_$BOOST_VER.7z ]] || wget https://boostorg.jfrog.io/artifactory/main/release/$BOOST_VER_DOT/source/boost_$BOOST_VER.7z;
...
do
    ...
    # define paths
    GCC_PATH=/c/OSRC/gcc-build/msys64/home/arkay7777/mingw-gcc-${GCC_VER}/${BUILD_ARCH}-${GCC_VER_CODE}-${GCC_THREAD_EXCEPT_OSTYPE}-rt_${MINGW64_CRT}-rev${REV}/${MINGW_DIR_32OR64}
    echo $GCC_PATH
    ...
    echo "Running boost bootstrap.sh for ${BUILD_ARCH}-${GCC_THREAD_EXCEPT_OSTYPE}-${BITS}bit"
    ...
    # ./bootstrap.sh --with-toolset=gcc --prefix="$BOOST_DIR" --includedir="$BOOST_DIR/include" --with-python-root="C:/OSRC/Anaconda3" --with-python="C:/OSRC/Anaconda3/python.exe" 1>>${BOOST_BUILD_LOG}
    ...
    # build boost
    echo "Running boost b2.exe for ${BUILD_ARCH}-${GCC_THREAD_EXCEPT_OSTYPE}-${BITS}bit"
    ...
    if [[ $BUILD_ARCH == "i686" ]] && [[ $BITS == "64" ]]; then
        ...
        DIFF_FILE=./boost_${BOOST_VER}_gcc${GCC_VER_CODE}_${GCC_THREAD_EXCEPT_OSTYPE_DIR}.diff
    ...
    echo "Installing boost-${BOOST_VER}-${BITS}bit in ${BUILD_ARCH}-${GCC_THREAD_EXCEPT_OSTYPE}"
    ...
    if [[ $THREAD == "posix" ]] && [[ $BITS == "32" ]]; then
        echo "configuration: boost build install v3.sh, 32-bit mt, <thread> from pthreads"           >> $GCC_PATH/build-info.txt
    elif [[ $THREAD == "posix" ]] && [[ $BITS == "64" ]]; then
        echo "configuration: boost build install v3.sh, 64-bit mt, <thread> from pthreads"           >> $GCC_PATH/build-info.txt
    elif [[ $THREAD == "win32" ]] && [[ $BITS == "32" ]]; then
        echo "configuration: boost build install v3.sh, 32-bit mt, <thread> from mingw-std-threads"  >> $GCC_PATH/build-info.txt
    elif [[ $THREAD == "win32" ]] && [[ $BITS == "64" ]]; then
        echo "configuration: boost build install v3.sh, 64-bit mt, <thread> from mingw-std-threads"  >> $GCC_PATH/build-info.txt
    fi
