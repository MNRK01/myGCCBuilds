...
do
    ...
    # define paths
    GCC_PATH=/home/arkay7777/mingw-gcc-${GCC_VER}/${BUILD_ARCH}-${GCC_VER_CODE}-${GCC_THREAD_EXCEPT}-rt_${MINGW_CRT}-${REV}/${MINGW_DIR_32OR64}
    ...
    LIBICU_DIR=/c/OSRC/libicu-${ICU_VER_CODE}/libicu_${ICU_VER_CODE}_gcc${GCC_VER_CODE}_${BUILD_ARCH}_${GCC_THREAD_EXCEPT_DIR}_x${BITS}_${REV}
    ...
    # configure and build
    echo "Starting ICU4C build for gcc${GCC_VER_CODE}_${BUILD_ARCH}_${GCC_THREAD_EXCEPT_DIR}_x${BITS}."
    ...
    if [[ $BUILD_ARCH == "i686" ]] && [[ $BITS == "64" ]]; then
        ...
        DIFF_FILE=/c/OSRC/libicu-${ICU_VER_CODE}/icu/libicu_${ICU_VER_CODE}_gcc${GCC_VER_CODE}_${THREAD}_${EXCEPT}.diff
    ...
    if [[ $THREAD == "posix" ]]; then
        echo "configuration: libicu build install v2.sh, <thread> from pthreads"                                                     >> $GCC_PATH/build-info.txt
    elif [[ $THREAD == "win32" ]]; then
        echo "configuration: libicu build install v2.sh, <thread> from mingw-std-threads"                                            >> $GCC_PATH/build-info.txt
    fi
